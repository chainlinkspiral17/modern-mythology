# ChristianIceGauntletHost.gd
# ════════════════════════════════════════════════════════════════
# Host script that wires the Tarot Gauntlet for THE STAR
# (Glass Skin / Christian Ice — Glass Skin) into
# christian_ice_co.tscn. Sister to DinerGauntletHost +
# CathedralGauntletHost + BungalowGauntletHost +
# RiverboatGauntletHost.
#
# Board-space positions match build_christian_ice_co.py geometry.
...
func _ready() -> void:
	...
	if _player == null or not is_instance_valid(_player):
		push_warning("[ChristianIceGauntletHost] Player path '%s' not found" % player_path)
	...
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if k.ctrl_pressed:
			...
			if k.keycode == KEY_F10:
				...
				launch_the_notice_in_the_picayune()
...
func position_player_at(space_id: String) -> void:
	...
	if entry == null:
		push_warning("[ChristianIceGauntletHost] Unknown space: %s" % space_id)
		return
...
func launch_glass_skin() -> void:
	_launch_scenario("star", "christian_ice_co", "tbd_star",
	                 "glass_skin", false)


func launch_the_last_sunday() -> void:
	_launch_scenario("star", "christian_ice_co", "tbd_star",
	                 "the_last_sunday", false)


func launch_the_notice_in_the_picayune() -> void:
	_launch_scenario("star", "christian_ice_co", "tbd_star",
	                 "the_notice_in_the_picayune", true)
