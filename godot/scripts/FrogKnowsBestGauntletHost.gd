# FrogKnowsBestGauntletHost.gd
# ════════════════════════════════════════════════════════════════
# Host script that wires the Tarot Gauntlet for THE WORLD
# (Frog Knows Best — Loop Completed) into
# frog_knows_best.tscn. Sister to DinerGauntletHost +
# CathedralGauntletHost + BungalowGauntletHost +
# RiverboatGauntletHost.
#
# Board-space positions match build_frog_knows_best.py geometry.
...
func _ready() -> void:
	...
	if _player == null or not is_instance_valid(_player):
		push_warning("[FrogKnowsBestGauntletHost] Player path '%s' not found" % player_path)
	if auto_position_on_ready:
		position_player_at("retail_counter")
	...
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if k.ctrl_pressed:
			if k.keycode == KEY_F10:
				...
				launch_the_year_ago_first_open()
			elif k.keycode == KEY_F12:
				launch_loop_completed()
...
func position_player_at(space_id: String) -> void:
	...
	if entry == null:
		push_warning("[FrogKnowsBestGauntletHost] Unknown space: %s" % space_id)
		return
...
func launch_loop_completed() -> void:
	_launch_scenario("world", "frog_knows_best", "tbd_world",
	                 "loop_completed", false)


func launch_the_walk_through_with_the_new_tenants() -> void:
	_launch_scenario("world", "frog_knows_best", "tbd_world",
	                 "the_walk_through_with_the_new_tenants", false)


func launch_the_year_ago_first_open() -> void:
	_launch_scenario("world", "frog_knows_best", "tbd_world",
	                 "the_year_ago_first_open", true)
