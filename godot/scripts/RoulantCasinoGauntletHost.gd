# RoulantCasinoGauntletHost.gd
# ════════════════════════════════════════════════════════════════
# Host script that wires the Tarot Gauntlet for THE WHEEL OF FORTUNE
# (TBD — The House Edge) into
# le_roulant_casino.tscn. Sister to DinerGauntletHost +
# CathedralGauntletHost + BungalowGauntletHost +
# RiverboatGauntletHost.
#
# Board-space positions match build_le_roulant_casino.py geometry.
...
func _ready() -> void:
	...
	if _player == null or not is_instance_valid(_player):
		push_warning("[RoulantCasinoGauntletHost] Player path '%s' not found" % player_path)
	if auto_position_on_ready:
		position_player_at("roulette_table")
	...
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if k.ctrl_pressed:
			...
			if k.keycode == KEY_F10:
				launch_the_after_hours_count()
			elif k.keycode == KEY_F11:
				launch_the_service_call_saturday()
			elif k.keycode == KEY_F12:
				launch_the_house_edge()
...
func position_player_at(space_id: String) -> void:
	...
	if entry == null:
		push_warning("[RoulantCasinoGauntletHost] Unknown space: %s" % space_id)
		return
...
func launch_the_house_edge() -> void:
	_launch_scenario("wheel_of_fortune", "le_roulant_casino", "tbd_wheel",
	                 "the_house_edge", true)


func launch_the_after_hours_count() -> void:
	_launch_scenario("wheel_of_fortune", "le_roulant_casino", "tbd_wheel",
	                 "the_after_hours_count", false)


func launch_the_service_call_saturday() -> void:
	_launch_scenario("wheel_of_fortune", "le_roulant_casino", "tbd_wheel",
	                 "the_service_call_saturday", true)
...
func open_scenario_picker() -> void:
	...
	add_child(picker)
	var entries: Array = [
		{"launch_fn": Callable(self, "launch_the_after_hours_count"), "title": "THE AFTER-HOURS COUNT", "subtitle": "Le Roulant Casino · 2:38 AM · the count room · doors are locked", "difficulty": "easy"},
		{"launch_fn": Callable(self, "launch_the_house_edge"), "title": "THE HOUSE EDGE", "subtitle": "Le Roulant Casino · 11:11 PM Friday · the night the wheel was off", "difficulty": "medium"},
		{"launch_fn": Callable(self, "launch_the_service_call_saturday"), "title": "THE SERVICE CALL · SATURDAY", "subtitle": "Le Roulant Casino · 10:14 AM Saturday · the wheel-service technician on the floor", "difficulty": "hard"},
	]
