# ChapelGauntletHost.gd
# ════════════════════════════════════════════════════════════════
# Host script that wires the Tarot Gauntlet for THE LOVERS
# (Sanctuary scenarios — Sanctuary on Cursed Ground) into
# roadside_chapel.tscn. Sister to DinerGauntletHost +
# CathedralGauntletHost + BungalowGauntletHost +
# RiverboatGauntletHost.
#
# Board-space positions match build_roadside_chapel.py geometry.
...
func _ready() -> void:
	...
	if _player == null or not is_instance_valid(_player):
		push_warning("[ChapelGauntletHost] Player path '%s' not found" % player_path)
	if auto_position_on_ready:
		position_player_at("altar")
...
func _input(event: InputEvent) -> void:
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if k.keycode == KEY_F12 and k.ctrl_pressed:
			launch_sanctuary_on_cursed_ground()
...
func position_player_at(space_id: String) -> void:
	...
	if entry == null:
		push_warning("[ChapelGauntletHost] Unknown space: %s" % space_id)
		return
...
func launch_sanctuary_on_cursed_ground() -> void:
	if _game != null and is_instance_valid(_game):
		print("[ChapelGauntletHost] Gauntlet already running.")
		return
	var ps: PackedScene = load(launch_scene_path) as PackedScene
	if ps == null:
		push_warning("[ChapelGauntletHost] Could not load %s" % launch_scene_path)
		return
