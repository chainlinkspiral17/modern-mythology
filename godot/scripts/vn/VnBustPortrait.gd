...
# jaw · 3 curly/afro · 4 bald · 5 cap · 6 feminine long.
const _OVERRIDES: Dictionary = {
	...
	"sam":         {"hair_style": 6, "beard": false},
	"sam_miller":  {"hair_style": 6, "beard": false},
	...
	"graciela":       {"hair_style": 2, "hair_color": 5, "beard": false,
					   "glasses": "regular"},
	"graciela_ramos": {"hair_style": 2, "hair_color": 5, "beard": false,
					   "glasses": "regular"},
	...
	"steamboat":     {"hair_style": 2, "hair_color": 5, "beard": true},
	...
	"tem":     {"hair_style": 6, "beard": false},
	"kai":     {"hair_style": 0, "beard": false},
	"lena":    {"hair_style": 6, "hair_color": 1, "skin_tone": 2, "beard": false},
	"finn":    {"hair_style": 1, "beard": false},
	"per":     {"hair_style": 0, "hair_color": 5, "beard": true},
	"cale":    {"hair_style": 1, "beard": false},
	"hans":    {"hair_style": 0, "hair_color": 5, "beard": true},
...
static func _build(key: String, fam: String, accent: Color, frame: String) -> ImageTexture:
	...
	for y in range(10, 40):
		...
		for x in range(20 + inset, 41 - inset):
			var c: Color = skin
			if x <= 21: c = skin_dk                     # shadow side
			elif x >= 38: c = skin_lt                   # rim-light side
			_put(img, x, y, c)
	_hspan(img, 23, 37, 38, skin_dk)                    # jaw shade
	for y in range(24, 29):                             # ears
		_hspan(img, 18, 19, y, skin)
		_hspan(img, 41, 42, y, skin)
	_put(img, 19, 26, skin_dk)
	_put(img, 41, 26, skin_dk)
