# SolenadeGardenGauntletHost.gd
# ════════════════════════════════════════════════════════════════
# Host script that wires the Tarot Gauntlet for THE SUN
# (Frank — dust motes — Dust Motes) into
# solenade_garden.tscn. Sister to DinerGauntletHost +
# CathedralGauntletHost + BungalowGauntletHost +
# RiverboatGauntletHost.
#
# Board-space positions match build_solenade_garden.py geometry.
...
func _ready() -> void:
	...
	if _player == null or not is_instance_valid(_player):
		push_warning("[SolenadeGardenGauntletHost] Player path '%s' not found" % player_path)
	if auto_position_on_ready:
		position_player_at("central_oak")
	...
	if event is InputEventKey and event.pressed and not event.echo:
		...
		if k.ctrl_pressed:
			...
			if k.keycode == KEY_F10:
				launch_the_ten_am_watering()
			elif k.keycode == KEY_F11:
				launch_the_shared_bench()
...
func position_player_at(space_id: String) -> void:
	...
	if entry == null:
		push_warning("[SolenadeGardenGauntletHost] Unknown space: %s" % space_id)
		return
...
func launch_dust_motes() -> void:
	_launch_scenario("sun", "solenade_garden", "tbd_sun",
	                 "dust_motes", false)


func launch_the_ten_am_watering() -> void:
	_launch_scenario("sun", "solenade_garden", "tbd_sun",
	                 "the_ten_am_watering", false)


func launch_the_shared_bench() -> void:
	_launch_scenario("sun", "solenade_garden", "tbd_sun",
	                 "the_shared_bench", true)
...
func open_scenario_picker() -> void:
	...
	add_child(picker)
	var entries: Array = [
		{"launch_fn": Callable(self, "launch_dust_motes"), "title": "DUST MOTES", "subtitle": "Solenade Memorial Garden · 2:18 PM · the longest afternoon in June", "difficulty": "easy"},
		{"launch_fn": Callable(self, "launch_the_ten_am_watering"), "title": "THE 10 AM WATERING", "subtitle": "Solenade Memorial Garden · 10:14 AM Saturday · the day Frank asked Mr Bouchon if he could help", "difficulty": "easy"},
		{"launch_fn": Callable(self, "launch_the_shared_bench"), "title": "THE SHARED BENCH", "subtitle": "Solenade Memorial Garden · 2:04 PM Wednesday · six weeks after the Saturday watering · another widower is on Frank's bench", "difficulty": "hard"},
	]
