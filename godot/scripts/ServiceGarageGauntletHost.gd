# ServiceGarageGauntletHost.gd
# ════════════════════════════════════════════════════════════════
# Host script that wires the Tarot Gauntlet for THE CHARIOT
# (TBD (the drive) — Old Lacombe) into
# lacombe_service_garage.tscn. Sister to DinerGauntletHost +
# CathedralGauntletHost + BungalowGauntletHost +
# RiverboatGauntletHost.
#
# Board-space positions match build_lacombe_service_garage.py geometry.
...
func _ready() -> void:
	...
	if _player == null or not is_instance_valid(_player):
		push_warning("[ServiceGarageGauntletHost] Player path '%s' not found" % player_path)
	if auto_position_on_ready:
		position_player_at("bay_lift")
...
func position_player_at(space_id: String) -> void:
	...
	if entry == null:
		push_warning("[ServiceGarageGauntletHost] Unknown space: %s" % space_id)
		return
...
func launch_old_lacombe() -> void:
	if _game != null and is_instance_valid(_game):
		print("[ServiceGarageGauntletHost] Gauntlet already running.")
		return
	var ps: PackedScene = load(launch_scene_path) as PackedScene
	if ps == null:
		push_warning("[ServiceGarageGauntletHost] Could not load %s" % launch_scene_path)
		return
