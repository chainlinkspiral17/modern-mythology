...
def prop():
    """A prop canvas: fully transparent, so whatever is drawn onto it
    keeps its own silhouette against the ground beneath."""
    return [TRANSPARENT] * (W * H)


def blob(data, cx, cy, rx, ry, c, squash=1.0):
    """Filled ellipse — the workhorse for organic shapes (canopies,
    boulders, stumps). Nothing in nature is a square."""
    for y in range(H):
        for x in range(W):
            dx = (x - cx) / max(rx, 0.001)
            dy = ((y - cy) * squash) / max(ry, 0.001)
            if dx * dx + dy * dy <= 1.0:
                data[y * W + x] = c


def blob_edge(data, cx, cy, rx, ry, c, squash=1.0, band=0.28):
    """Rim of an ellipse — a darker outline that separates a prop from
    the ground without a hard box edge."""
    for y in range(H):
        for x in range(W):
            dx = (x - cx) / max(rx, 0.001)
            dy = ((y - cy) * squash) / max(ry, 0.001)
...
def emit_all():
    ...
    for name, fn in TILES.items():
        ...
        # Index 0 is the transparent slot. A tile function may mark a
        # pixel TRANSPARENT with the sentinel -1; everything else
        # shifts up by one into the opaque range. Props use this to
        # have a SILHOUETTE instead of being a 16x16 box — they are
        # drawn over the zone's ground tile by CampOverworld.
        pal_out = [""] + list(pal_hex)
        data_shifted = [0 if c < 0 else c + 1 for c in data]
        obj = {
            "id": name,
