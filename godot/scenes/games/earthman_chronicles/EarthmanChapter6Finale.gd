...
func _render_ending_intro() -> void:
	...
	# HeroImage · per-ending art · all six endings have one
	var hero_file: String = "ending_" + _selected_ending
	if _selected_ending == "the_correction":
		hero_file = "correction_page"
	var hero := HeroImage.new()
	if hero.load_from("res://resources/games/vol7/earthman_chronicles/hero_images/" + hero_file + ".json"):
