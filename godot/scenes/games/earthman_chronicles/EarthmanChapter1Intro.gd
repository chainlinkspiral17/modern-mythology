...
func _build_frame() -> void:
	...
	# HeroImage · Jack's Pasadena workshop · top-right of the intro
	var hero := HeroImage.new()
	if hero.load_from("res://resources/games/vol7/earthman_chronicles/hero_images/pasadena_workshop.json"):
	...
	# Added first so the panel draws on top of it, leaving only the ring.
