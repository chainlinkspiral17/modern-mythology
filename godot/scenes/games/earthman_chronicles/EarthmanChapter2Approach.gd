...
func _build_frame() -> void:
	...
	# Bezel ring — an amber frame sitting 10px proud of the panel on all
	# sides, so the cortex box reads as inset inside a ringed border
	# (the "art rings the outside of the bounds" note). Added first so
	# the panel draws on top of it, leaving only the 10px ring showing.
...
func _on_choice_selected(choice: Dictionary) -> void:
	...
	# Traditional duel · the real fight
	if bool(sets.get("combat_pending", false)):
		_run_state.erase("combat_pending")
		var choices_log_d: Array = _run_state.get("choices_log", [])
		choices_log_d.append({"chapter": 2, "beat": _beat_idx, "choice_id": String(choice.get("id", ""))})
		_run_state["choices_log"] = choices_log_d
		_launch_combat("hel_velli_duel")
		return
