...
const SD2_HOST_SCENE := "res://scenes/games/spiderdrops_2/SpiderdropsTwoHost.tscn"
const SB_HOST_SCENE  := "res://scenes/games/salmonberry/SalmonberryHost.tscn"
...
func _on_picked(stick_id: String, manager_mode: bool = false) -> void:
	...
	elif stick_id == "spiderdrops_2":
		_open_host_spiderdrops_2()
	elif stick_id == "salmonberry":
		_open_host_salmonberry()
...
func _open_host_salmonberry() -> void:
	if _shelf != null:
		_shelf.queue_free()
		_shelf = null
	_current_stick_id = "salmonberry"
	_host = load(SB_HOST_SCENE).instantiate()
	_host.quit_to_shelf.connect(_open_shelf)
	_host.finished.connect(_on_host_finished)
	add_child(_host)


func _open_host_spiderdrops_2() -> void:
	if _shelf != null:
		_shelf.queue_free()
		_shelf = null
	_current_stick_id = "spiderdrops_2"
	_host = load(SD2_HOST_SCENE).instantiate()
	_host.quit_to_shelf.connect(_open_shelf)
	_host.finished.connect(_on_host_finished)
	add_child(_host)
