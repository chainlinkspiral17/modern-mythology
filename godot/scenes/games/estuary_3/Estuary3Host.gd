...
func _render_debug() -> void:
	...
	lines.append("")
	lines.append("Where the cart left you: %s" % _current_act.replace("_", " "))
	lines.append("Nights worked this act: %d" % int(_run_state.get("night_index", 0)))
	lines.append("What the summer has decided so far, in the cart's own shorthand:")
	lines.append("  %s" % JSON.stringify(_run_state.get("canon_vars", {})))
	lines.append("  %s" % JSON.stringify(_run_state.get("lore_tokens_pending", [])))
	lines.append("")
	lines.append("── SERVICE CARD ──")
	lines.append("The cart holds your summer where you left it.  Nothing")
	lines.append("is lost by looking.  BACK TO SHELF sets the cart down")
	lines.append("without ending the run.")
	_debug_label.text = "\n".join(Array(lines))
