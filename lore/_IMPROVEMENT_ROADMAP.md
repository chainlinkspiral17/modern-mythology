...
| Scene direction · coverage rotation (2026-08-04) | 3 (rotation live; 13 locales carry 4-setup decks — 52 authored. Draft 3 also found + fixed the ROW-major transpose bug: all 49 pre-existing matrix markers aimed mirrored/dutched; see VN playbook 2026-08-08) | Deck screenshots of the setups — the framing math is verified to <0.5°, the framing TASTE is unreviewed; then remaining single-angle rooms (roberts_house, bar_interior, hospital_room if they clear ~8 uses) get decks; convert generator to emit position+rotation euler form (diff-readable, convention-proof) |
