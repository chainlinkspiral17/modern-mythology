...
hum sinks to 35% for its length, then both return. `bed(trim=)`
brings the set within 4 dB before normalization. 304 cues across 184
scenes. NEXT (Deck): the once-per-scene rate (three stingers in a
vol 6 scene's first minute), the 0.35 floor, the 2026-09-11 beds
re-rendered with the enveloped `rain` voice, the 27 unreachable themes as scenes are written,
and whether gauntlet visitors should cue from the board.
...
| **Signatures (2026-09-12)** | **1** (19 character themes authored — every theme whose character is shown or speaks in its volume; `faust3`→`faust`; cue once per scene on first show OR first line, one-shot that hands the bed back; the duck inverts under a signature — hum to 35%, theme to full; `bed(trim=)` levels the set; 304 cues / 184 scenes) | Deck: the once-per-scene rate, the 0.35 floor, the `rain` voice's new envelope on the older beds (one re-render), the translations that miss (Rick, Carl); then the 27 unreachable themes as their scenes are written; gauntlet visitors cued from the board |
