...
vol 6 scene's first minute), the 0.20 floor, the 27 unreachable themes as scenes are written,
...
| **Signatures (2026-09-12)** | **1** (19 character themes authored — every theme whose character is shown or speaks in its volume; `faust3`→`faust`; cue once per scene on first show OR first line, one-shot that hands the bed back; the duck inverts under a signature — hum to 20%, theme to full; `bed(trim=)` levels the set; 304 cues / 184 scenes) | Deck: the once-per-scene rate, the 0.20 floor, the translations that miss (Rick, Carl); then the 27 unreachable themes as their scenes are written; gauntlet visitors cued from the board |
