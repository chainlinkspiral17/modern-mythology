# VOL 10 · ROFLCOPTR · Wiki

The synthesizing entry point for vol 10 of *Modern Mythology* — the
final volume, working title **ROFLCOPTR**, which begins as **Drift
Wood**, a high-school newspaper comic strip in Small Wood, Oregon,
1997. Twenty-three years of a public strip, six of a private one,
and the man who drew them. **Drift Wood is everything before the
burnout; ROFLCOPTER is the personal voyage after it.**

Status: **outlining / prep** (September 2026). Nothing here is
engine-consumed yet. Source of the tone and scope is the user's
brief, transcribed in full at `drift_wood/_SOURCE_BRIEF.md`.

## What vol 10 is

A bittersweet chronicle of arrested development, shifting media
landscapes, and Pacific Northwest melancholy. **Arthur "Woody"
Finch** is Small Wood High's golden boy in 1997 — editor of *The
Timberline*, an effortless draftsman, voted Most Likely to Win a
Pulitzer. He never leaves. He draws *Drift Wood* about the town
for twenty-three years — every two weeks in the school paper, five
days a week in the college years, then six dailies and a Sunday on
a newspaper cadence for sixteen more — and is known the whole time
for a one-week meme parody he drew at 3:42 AM in 2006. The Sunday
watercolors become the best work a niche crowd of illustrators
knows of. He burns out the way Watterson did and stops, December
2020, without announcing it. Then he keeps drawing, privately,
pages instead of strips, and names them *ROFLCOPTER* — the meme's
name, spelled right, taken back. The personal voyage ends on a
cedar log at the mouth of the bay at dawn, April 2027.

**We don't see the author, but we feel him.** The volume is the
strip, read in order, with a foreword before and an afterword
after — the only prose, and the only place Arthur is seen. In the
panels there is only **Wood**, the character he draws himself as,
and the tragicomedy is the distance between what Wood is shown to
be and what the strips betray: the delusions of grandeur, the
failing through life, the hopes, dreams, fears and epiphanies,
rendered through the work. If there is a happy ending, it's that
the work is published — and that the specter of his own life is
foreshadowed in its pages. See `drift_wood/_AUTHORIAL_RULE.md` and
`drift_wood/foreword_afterword.md`.

The volume's thesis, in the brief's words: *talent without
discipline is just a very articulate form of self-sabotage* — and
then, thirty years later, the reversal: *ten thousand days of
noticing things. Nothing gets wasted unless you weren't looking.*

Tonal register: **damp / observant / screwball-over-flat.** Rapid
literary wit bouncing off monosyllabic PNW realism. Arthur delivers
eight sentences on the commodification of nostalgia; Todd asks
cheddar or pepper jack. Slower than vol 7 in the Sunday pages,
faster than anything in the saga in the dailies.

### The works-within-works shape

This volume, like the saga, is nested and loose:

- **The strip** — *Drift Wood* (1997–2020). The public artifact.
  Biweekly, then weekday-daily, then dailies and Sundays.
- **The meme** — *ROLFCOPTR* (one week in November 2006). The
  albatross. Not the strip; the thing people think the strip is.
- **The personal voyage** — *ROFLCOPTER* (2021–2027). The private
  work: pages, no grid, no readers, the whole town and everywhere
  the strip never went. The masterwork, unpublished.
- **The phantom graphic novel** — the work Arthur never finishes,
  which changes genre every four years. A work that exists only as
  Box 4B and as the thing the strip is *not*.
- **The retrospective** — the 2027 Seattle publisher's complete
  edition, with a foreword and an afterword that tell the author's
  life and hardships. The book the reader is holding.
- **The volume** — *is* the retrospective. Foreword, the strip in
  runs, the private pages, afterword. No scenes of the life; only
  the record of it and two essays about the man who kept it.
- **Vol 2, *Small Wood Volumes*** — a notebook found in a copy shop
  and finished by someone else. The saga's first Small Wood work is
  itself a found document. Vol 10 closes the same town from the
  other end of the shelf. This is felt, not enforced.

The spellings are deliberate. **ROLFCOPTR** is the meme (misspelled,
3:42 AM, 2006). **ROFLCOPTER** is the private work (spelled right,
2021). **ROFLCOPTR** / *ROFLcopter* is the volume (the engine's
main-menu title, the outside-the-fiction name). He couldn't get the
meme right; he gets it right when nobody is watching.

## Quick reference · who and what

### The primary cast

*Everyone below is known through the strip and the bookends only.
In the panels, Arthur is "Wood"; Chloe and Todd are the strip's
Chloe and Gully. The biographies are what the reader infers.*

| Character | What |
|---|---|
| **Wood** | The strip's lead. Arthur as he draws himself — taller in 1997, two-thirds size after the meme, absent for months in 2021, then from behind, then a reflection. The gap between Wood and Arthur is the volume's engine · `drift_wood/_AUTHORIAL_RULE.md` |
| **Arthur "Woody" Finch** | Born ~1980 · the protagonist and the strip's author · 1997 editor of *The Timberline* · turns down the PNCA scholarship in 2000 · Heceta Frame & Matting on Highway 101 (2000s) · night auditor at the Driftwood Motel (2010s–2027) · childhood bedroom → garage apartment behind the cedar-shake tavern · 1993 Subaru Legacy wagon, oil-burning · signs *A. Finch — Small Wood, OR* with a tiny gull or crab · `drift_wood/arthur_finch.md` |
| **Chloe Sterling** | Arthur's intellectual equal and harshest mirror · *Timberline* culture editor, smudged eyeliner, battered Discman · leaves (1999 / autumn 2000, see below) · Seattle, Chicago, Europe, Portland · senior strategist at a boutique design agency · marries Julian in July 2014 (San Juan Islands), divorces quietly in 2021 · sits on Todd's dock with a Rainier in 2024 · `drift_wood/chloe_sterling.md` |
| **Todd "Gully" Gulliver** | The anchor · backwards cap, work boots, ten-second delay · inherits a shingle shack by the boat launch from his uncle in 2003 → **Gully's Tackle & Roast**, bait shop + espresso bar, the town's living room · marries Maria (county nurse) 2009, ranch house on a half-acre of hemlock, two kids · lends Arthur money under thin pretexts · pins Arthur's pencil roughs above the register · `drift_wood/todd_gulliver.md` |
| **Barnaby** | Low-slung terrier-hound mix, folded velvet ears, smells of dead bull kelp · found tied to a dumpster behind the bowling alley, November 1999 · does not talk, no thought bubbles · dies at sixteen, late 2014 (Arc 4) · `drift_wood/barnaby.md` |
| **Barnaby II** | Three-legged wirehair terrier/border pup · arrives in a box stamped ALICE'S WHOLESALE BAIT & SMOKED MEATS · Todd's note: *His name is not my problem* · sentry through the 2027 finale · `drift_wood/barnaby.md` |
| **Julian** | Chloe's husband, 2014–2021 · senior interface architect at a tech firm · affable, wealthy, not a villain |
| **Maria Gulliver** | Todd's wife · pragmatic county nurse |
| **The editor** | Young, from the boutique Seattle publisher, 2026–27 · expects a recluse, finds a man with a routine · writes the foreword (lean) · appears in the strip once, in the brief's Box 4B daily · unnamed (reserved) |
| **The afterword's author** | Chloe (lean). The one prose voice from inside the strip's cast · `drift_wood/foreword_afterword.md` |
| **Arthur's father** | Gets sick in Phase Two (mid-2000s) · otherwise reserved |

### The concept-sheet variant (September 2026, work in progress)

A concept sheet labels the character **"Wood"**, gives him a
receding hairline, a blue plaid flannel over a band tee, ripped
jeans and a tablet, and pairs him with a **small rainbow songbird**
rather than a dog. It also introduces a yellow **eye sign** on a
street pole, a fish weathervane, and a **moss-grown stone head**
beside a bench. See `drift_wood/_CONCEPT_ART.md`. The bird and the
dog are not reconciled yet; that is the largest open question in
the cast.

### Vol 2 / vol 6 / vol 7 figures who could be in the frame

Small Wood is Smolvud (see Echoes). None of the following are in
the brief; all are available as background in the strip's panels,
never as plot.

- **Mrs. Marian Gable** — the town librarian, active in 1997,
  retired by vol 6's window. *The Timberline*'s back issues would
  be in her library. The strip's alt-weekly run would be in her
  archive whether she filed it or not.
- **Sal Carratura** — Pizza Pirate on Cypress St since 1991. The
  laundromat with the free alt-weekly rack is, plausibly, the one
  through the door in Pizza Pirate's back hallway.
- **Wagner's Hardware** — open 1962–2011, on the corner. Arthur
  would have bought X-Acto blades there until it closed, then
  ordered them online, then bought them at Board Lords' neighbor.
- **Otis Vandermeer** — radio-engineering veteran with a garage
  workshop off 101. Plausibly the man who fixed the Subaru's
  radio.
- **Lena Vargas, Kai, Petra, Wren** — the vol 7 cast are the kids
  in the background of Arthur's Phase Four panels. Lena publishes
  *Static Truths* in August 2025; Arthur is drawing the same
  streets that month.
- **Aria Dean** — a yellow rainslicker in a crowd panel, once, if
  the writer wants it. Never more.

## Locations

### Small Wood proper

- **Small Wood High** — *The Timberline* newsroom. The journalism
  classroom where Todd draws caricatures in the margins.
- **The laundromat** — the free alt-weekly rack where *Drift Wood*
  migrates after graduation.
- **Heceta Frame & Matting** — the art-and-framing shop on Highway
  101. Arthur's backroom, foam core, a dull utility knife. The
  rain alley with the spackle buckets.
- **The cedar-shake tavern + the garage apartment** — Arthur's
  home from the 2000s on. Casement window facing the gravel alley
  behind the tavern. Corrugated tin roof. Douglas-fir floorboards.
  Cinderblock bookcase. The tavern is unnamed in the brief
  (reserved — it may or may not be Club # / Sharp).
- **Gully's Tackle & Roast** — Todd's shingle shack by the boat
  launch. Two-group espresso machine, nightcrawlers, smoked salmon
  chowder, the corkboard above the register. The dock out back.
- **The Driftwood Motel** — Arthur's night-audit desk. The ice
  machine's hum.
- **The bowling alley** — its neon sign in the rain (a Sunday
  landscape); the dumpster behind it where Barnaby was tied.
- **The abandoned cedar mill** — shuttered 1994. Arthur and Chloe
  walk it in 2008.
- **The Greyhound stop** — southbound to Eugene to connect north.
- **The county clinic** — Arc 4.
- **Todd's ranch house** — half an acre of hemlock.

### The coast around it

Real geography, used straight: **Alsea Bay** (the mouth of the bay,
the finale's vista, the mudflats of "The Carry"), **Newport** (the
24-hour diner, 2018), **Waldport** (the muffler shop), **Toledo**
(the log dump where Barnaby II was found), **Beverly Beach** (the
sand dollar on the sill), **Coos Bay** (the timber camp, per the
bio), **Heceta** (the frame shop's name), **Highway 101** (the
serpentine bend the bus disappears around), **Eugene**, the **San
Juan Islands** and **Puget Sound** (Arc 3), **Vancouver Island**'s
lights across the water.

Vol 7 already places Smolvud's cannery on Alsea Bay and its tower
at old-Yachats. The map holds.

## The artifacts

### The strip · *Drift Wood* → *ROFLCOPTER*

Four eras, each with its own line and its own paper. Full spec in
`drift_wood/the_strip.md`.

| Era | Years | Line | Where it ran |
|---|---|---|---|
| I · The High School Zine Era | 1997–2003 | crow-quill + ballpoint on cardstock, xeroxed hard | *The Timberline* every two weeks; the college paper weekdays; the laundromat digest |
| II · The Webcomic Diaspora | 2004–2011 | brush-pen, muddy digital flats, rubbery line | Arthur's own site on a newspaper cadence; the ROLFCOPTR incident, Nov 2006; coastal weeklies from ~2008 |
| III · The Masterwork of Mediocrity | 2012–2020 | Derington-sharp brush and ink; Watterson wet-on-wet Sundays | web + coastal papers + a niche of illustrators; ends Dec 27, 2020 |
| IV · ROFLCOPTER · The Pacific Twilight | 2021–2027 | spare, knowing; muted ochre / slate / sea-foam; pages, no grid | nobody. The garage. Boxes 21–40 |

Two Sunday pages are fully specified, tier by tier, and are the
volume's visual canon: **"The Empty Chair"** (November 30, 2014)
and **"The Box"** (December 21, 2014).

Cadence, venues, the two sabbaticals, the 2018–2020 burnout and
the private era are in `drift_wood/publication_history.md`. The
edition — about 1,500 pages of finished material, a two-volume
physical set (*DRIFT WOOD*, *ROFLCOPTER*) and one digital edition,
selected from the ~10,000-strip archive — and the complete run
roughed arc by arc are in `drift_wood/_THE_COMPLETE_RUN.md`; every
year 1997–2027 is in `drift_wood/_SCOPE_AND_YEARS.md`.
Drafted strips for every era are in `drift_wood/sample_strips.md`.

### The phantom graphic novel

Rewritten since 1998. A gritty crime noir (2002), an
autobiographical indie travelogue (2008), a sprawling magical-
realist timber allegory (2016), the "serious timber-faller
allegory" he swears he'll return to after one week of the
ROLFCOPTR parody.
Lives in **Box 4B**, holding down a corner of the rug. Sixty pages,
2001–2003, mostly unreadable. The retrospective editor asks for it
first.

### The archive

Over ten thousand hand-drawn daily strips in cardboard banker's
boxes in the garage. The volume's physical body.

## Recurring motifs

- **The Sunday landscapes.** Every fourth or fifth Sunday, no
  jokes. Rain on the bowling alley's neon. Potholes on logging
  roads. Twenty minutes of sun through Sitka spruce.
- **The signature.** *A. Finch — Small Wood, OR*, dated, with a
  tiny gull or crab. On "The Empty Chair" there is only a flat
  stroke. On "The Box" there is a tiny three-legged scruff, asleep.
  The signature mark is the strip's emotional telemetry.
- **The suspended nib.** Ink loaded, a quarter-inch off the paper,
  the droplet refusing to fall. Arrested development as a drawing.
- **The box on the porch.** Todd's way of saying things.
- **Fried clams in silence.** The Arthur-and-Todd beat: eight
  sentences, then a monosyllable, then two men chewing in the rain.
- **The dog bed under the table.** Warm in the lamp's cone, then
  empty in the gray, then bypassed by the pup for the wastebasket.
- **The Subaru.** Doughnut spare, dread, oil.
- **Cedar floating out to sea.** The final signature's branch.
  Drift wood: the title was always the ending.

## Echoes · felt, not adhered to

The saga's continuity is a hum under the floor, not a ledger. The
following are echoes available to the writer. None is required.

- **Small Wood is Smolvud.** The vol 6 lore places Mrs. Gable, the
  Salty Tome, Wagner's Hardware and the cannery in "Small Wood";
  vol 7 places the same people and buildings in "Smolvud"; the 3D
  playbook calls Small Wood a Smolvud sub-area. Vol 10 uses the
  brief's name. The reader who has been through vol 7 will
  recognize the streets. Arthur never names the other name.
- **The strip as the third surface record.** Vol 6 has the Glitch
  Report and *NEWS FROM HARMONY CREEK*; vol 7 has *Static Truths*;
  vol 10 has thirty years of a daily strip drawn in the same town.
  If the substrate is in vol 10 at all, it is in what Arthur draws
  without knowing: the tower's glow on the ridge in a Sunday
  landscape, a crow on the seawall in three panels a decade apart,
  the Driftwood Motel's ice machine humming at a pitch the reader
  of vol 6 knows. The strip itself stays clean. Recommended, not
  decided.
- **The Fool's dog.** The Rider-Waite Fool has a small dog at his
  heels. Arthur is a Fool who never steps off the cliff, and
  Barnaby is the dog. The card-noticing that vol 7 does at chapter
  sixteen could, in vol 10, be a single Sunday page. Reserved.
- **Vol 2's found notebook.** *Small Wood Volumes* is narrated by
  someone whose family moved to Small Wood the summer after
  sophomore year, who knows the Elk Lodge, the grunion festival,
  Little Switzerland. Whether that narrator and Arthur were ever in
  the same cafeteria is a thing to feel, not to check.
- **The mill.** The brief says *the* mill shuttered in 1994; vol 6
  says the mills closed in slow succession; vol 7 has the Netarts
  Bay Lumber Mill standing abandoned. All three are true of a
  timber coast. Arthur's mill is the cedar mill in town.
- **The far horizon.** The vol 7 wiki holds the saga's endpoint as
  an image: a feral child, a ceramic frog, the ground where the
  riverboat was. With vol 10 declared the final volume, that image
  either lives inside vol 10 (an epilogue after April 2027, a
  diorama), or stays as the horizon beyond the last page. Not
  decided. See "Reserved."

## The brief disagrees with itself · allowed

The brief was written fast and warm and contradicts itself in
small ways. Under this saga's rules these are not errors to fix;
they are the strip's own memory being unreliable. Recorded so the
writer chooses on purpose:

| Fact | Version A | Version B | Lean |
|---|---|---|---|
| Arthur's age | 16–17 in 1997 (born ~1980); mid-forties in 2021–27 | "turns fifty" in the finale | Born 1980; the editor calls him fifty because that is the round number people use |
| Chloe leaves | for college, 1999 | boards the Greyhound for Seattle, autumn 2000 (Arc 1) | Both: left for college in 1999, was home for a summer, and the autumn 2000 bus is the one that stuck |
| Julian | product manager, Portland, 2014 | senior interface architect, Seattle, San Juan Islands wedding | Seattle; the pavilion on Puget Sound |
| Barnaby II arrives | December 21, 2014 ("The Box") | 2016, per the bio | December 2014; the Sunday page is the canon |
| Barnaby II's origin | log dump in Toledo (Todd's note) | timber camp near Coos Bay (bio) | Toledo, in Todd's grease pencil. Coos Bay is what Todd tells people |
| Barnaby's mix | terrier-hound | terrier-and-beagle | It's a dog from behind a bowling alley |
| Barnaby II's mix | wirehair terrier/border | three-legged wirehair pup | Same dog |
| The companion | a dog (brief) | a rainbow songbird (concept sheet) | **Open.** See `_CONCEPT_ART.md` |

## The vol 10 player canon

Reserved. No gallery game is designed. The saga's frame (every
volume has an authorial canon and a player canon) implies one. The
shape that fits the rule: **the player is the editor.** Forty boxes
in a garage, a page count, a deadline; the player chooses what the
retrospective includes and never meets the author either. See
`drift_wood/_ARC_OUTLINE.md`.

The engine currently lists vol 10 as *ROFLcopter* with the `glitch`
skin. The brief argues for a skin that **drifts with the eras** —
xerox grain, then CRT glow, then watercolor paper — or for the
strip's own panel grid as the presentation. Decision reserved.

## Reserved

- Whether the specter is literal — whether Arthur is alive when
  the book is published. Decides the afterword's tense. Nothing
  else needs it yet (`drift_wood/foreword_afterword.md`).
- Who writes the foreword and afterword (lean: the editor; Chloe).
- The hardships the afterword names that the strip never showed.
- The tavern's name.
- The editor's name.
- Arthur's father and mother; the childhood bedroom; when he moves
  to the garage.
- The bird, the eye sign, the stone head (concept sheet).
- Whether the substrate appears at all, and if so only in the
  Sunday landscapes.
- Whether the far-horizon image (frog, child, riverboat ground) is
  vol 10's last page.
- Vols 8 (*SCUMM*) and 9 (*Por Puesto*) have no lore yet. Vol 10 is
  outlined ahead of them because the user wants the ending in
  hand first.

## Reading order for someone catching up

1. **`drift_wood/_SOURCE_BRIEF.md`** — the brief, in full. Read it
   before anything else here.
2. **`drift_wood/_README.md`** — how the files are organized and
   what the strip's eras are.
3. **`drift_wood/_AUTHORIAL_RULE.md`** — we don't see the author.
   Read before writing any strip or scene.
4. **`drift_wood/foreword_afterword.md`** — the bookends, the
   ending, the foreshadowing index.
4b. **`drift_wood/style/`** — the style guides: characters by era,
    the eras, hero locations, objects. Read before drawing or
    generating anything.
5. **`drift_wood/the_strip.md`** — the four eras and the two
   fully-specified Sunday pages.
6. **`drift_wood/publication_history.md`** — the cadences, the
   names, the burnout, the personal voyage.
7. **`drift_wood/sample_strips.md`** — drafted strips per era,
   with the delusion-and-leak samples.
8. **`drift_wood/arthur_finch.md`** and the other three character
   files, read as inference documents.
9. **`drift_wood/_THE_COMPLETE_RUN.md`** — the edition (~1,500
   pages, two physical volumes and a digital one), the selection
   principle, and the complete run roughed arc by arc.
10. **`drift_wood/_SCOPE_AND_YEARS.md`** — every year 1997–2027, the
    per-year spine.
11. **`drift_wood/_ARC_OUTLINE.md`** — the book's structure as runs
    of strips and the proposed scene-id plan.
12. **`drift_wood/_CONCEPT_ART.md`** — the September 2026 sheet.
13. **`_VOL7_WIKI.md`** — only for the Smolvud echoes and the far
    horizon.
14. **Vol 2's title scene** — there is no vol 2 wiki; read
   `godot/resources/scenes/vol2/vol2_title.json` for the found-
   notebook frame if the nesting matters to the chapter at hand.

## Status

A rough batch of the full run exists, deepened twelve times: 1,024 strip scripts (browse them with `godot/tools/comic/comic_inspector.py`) in
`godot/tools/comic/strips/` from Drift Wood #1 (Sept 12, 1997) to
the final ROFLCOPTER page (April 25, 2027), every year and every
sequence represented, with the style guides in `drift_wood/style/`
and a tagged reference registry. The deepening pass has filled the nine
annual fixtures for every public year (2010's reprints excepted) and
the half-marked arcs of 2001–2003 and 2012–2013. Next: the
ROFLCOPTER sequences to Vol. II's weight, more selected Sundays, and
render. The brief is transcribed; the
reference files are drafted from it and from the user's notes on
cadence, the authorial rule, and the bookends; sample strips exist
for every era. No scene JSON exists. The engine's volume slot exists and is locked.

The chapter is held.
