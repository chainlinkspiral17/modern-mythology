...
pendants because of it (support 517); the eleventh took five more rooms to zero (support 447); the twelfth found the counter kit burying all its dressing 2 cm deep in 22 builders and took six more rooms to zero (support 371); the thirteenth took eleven more, including the two worst overlap rooms (the pit stop 26 clips → 0, the courthouse 10 → 0); the fourteenth took eleven more and fixed two more kits (the hospital bed's rails, the security camera); the fifteenth took the seven rooms at 7 and the three worst overlap rooms to zero — the Centro break room's whole kitchenette faced the wall, and the crumpled barn's automaton hung inside a solid block (support 123, overlap 120). The sixteenth found 43 WINDOWS IN 34 ROOMS BUILT INSIDE THEIR WALLS (the window kit only built toward the room on north and east walls) — every one now on its room face — and took the diner to zero again (support 75, overlap 81). The seventeenth found the same fault in the wall kit's BASEBOARDS — 398 in 79 rooms built inside their walls — now on the room face (119 walls also had theirs pointed out of the room), and took eight more rooms to zero (support 45, overlap 75). The eighteenth took the rooms at 3 and the kwik stop (two Slurpee machines in one spot; an end-cap display built into another) to zero (support 27, overlap 56). The nineteenth took the support count to ZERO across all 114 measured locales — nothing floats — and overlap to 34 (the kwik stop's NE corner re-planned; new auburn road's ditches had been hidden under its ground sheet). The twentieth took overlap to ZERO too — both gates now hold every builder at zero (any float or clip fails the suite) — after the recorder learned a blob's true squashed shape and exposed 48 blobs seated as if round. THE SUPPORT PASS's measurable targets are met; its next draft is the Deck: see it rendered, and fix what the gates cannot see. The twenty-first added a gate that runs every builder's REAL kit code (two builds had been dying in Blender unseen) and fixed the sheet's worst cameras (D'Ambrosio's stood outside the building). The twenty-second read the sheet's black frames: 32 key lights in 31 scenes pointed UP (now down, and gated), ~15 hand-built windows sat inside their walls, and the marker-aim gate had never been able to fail (four misaims fixed, gate repaired). The twenty-third found what the lens could not see: 22 keys shining sideways and 39 rims shining up (now gated), and GLASS — the pipeline has no alpha, and most glass cases were solid blocks with the product inside (the kwik stop's coolers hid 95 objects a door); the case kits and about twenty hand-built cases are open shells now, and the cabin's crow is seen through a window with no glass. The twenty-fourth read a sheet of white rooms: a Blender update's exporter had dropped the vertex colours of every GLB rebuilt that day — fixed in every export path, gated in the dry run and on the built GLBs, and the stale-build check now follows kit changes (kit fixes had never reached their callers). The twenty-fifth found the colours were there but WASHED (a linear/sRGB flip in the new exporter chain): every export now reads its GLB back and linearises COLOR_0 only when it came out unconverted, and each sheet carries a GLB diagnostic back. The twenty-sixth read that diagnostic: the colours in the files were correct (Blender 5.2) — Godot's import default material had stopped reading them; every locale now switches vertex colours on at load (VertexColorGuard), no rebuild needed. The twenty-seventh confirmed it on the sheet (flat frames 135 → 8, every room in colour, the open cases reading) and keyed the dark rooms: three rooms had no key at all, three night/dusk keys sat under the playbook band. The twenty-eighth found why the keys did nothing (the night mood's quantiser floors everything under 28/255 to black) and applied the playbook's ambient rule to eight sets; the frog tanks, the cake dome and the bourbon corner joined the glass pass. The twenty-ninth put the black night frames to the user: the dark moods now lift their shadows before the quantiser (shadow_lift: night, 3:47 am, dusk); every other mood is unchanged. The thirtieth confirmed the night frames read, and found every kit wall clock rendered as a blank disc (a solid rim in front of the dial) — the kit is layered and faces its wall now, all 44 clocks on their walls' room faces. The thirty-first worked the user's list: the ceiling "furniture" was the ceiling kit's dark stains (faint blotches now, 80 rooms); ~25 blocked doorways cleared, the misaligned ones closed to their frames (template rooms had cut 2–3 m gaps for 0.9 m doors); the sprinklers are impact heads, three a lawn, stepping their arcs; the five smallest bedrooms widened; 46 posters brought out of their walls.
